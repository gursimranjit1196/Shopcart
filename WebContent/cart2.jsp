...
ResultSet rs=st.executeQuery("select * from cart where username='"+x+"'");
	while(rs.next())
	{
			%>
		<div class="prod_alnmt"> 
	      ...
	      <div class="center_prods_box" >
	        <div class="product_title" style="text-align: center;font-size: 25px;text-decoration: underline;" ><%=rs.getString(3) %>       </div>
	        <div class="product_img"><img  class="fancybox"  src="sellerimage/<%=rs.getString(4) %>" data-big="sellerimage/<%=rs.getString(4) %>" height="173px" width="173px" border="0" /></div>
	        <div class="prod_price" style="text-align:center;font-size: 25px;"> Price:<span class="price"><%=rs.getString(5) %></span></div>
	        	        <div class="prod_button" style="color:#ff8a00; font-size: 23px; text-align: center; text-decoration: underline;"><a href="decidebuy.jsp?var1=<%=session.getAttribute("user")%>&var2=<%=rs.getInt(2) %>&var3=<%=rs.getString(3) %>&var4=<%=rs.getString(4) %>&var5=<%=rs.getString(5)  %>" type="hyperlink">Buy Now</a></div><br></br>
	        	         <div class="prod_button" style="color:#ff8a00; font-size: 23px; text-align: center; text-decoration: underline;"><a href="removecart.jsp?var1=<%=rs.getInt(2) %>" type="hyperlink">REMOVE</a></div>
	       </div>
	      </div>
